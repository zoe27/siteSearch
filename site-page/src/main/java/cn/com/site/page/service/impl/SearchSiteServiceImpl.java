 ...
 * @Description: TODO
 * @author 作者 zhoubin
 * @version 创建时间：2019年8月4日 上午10:33:32 SearchSiteServiceImpl.java
 * @since JDK 1.8
...
public class SearchSiteServiceImpl implements SearchService {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see cn.com.site.page.service.SearchService#searchByQuery(java.lang.String,
	 * int, int)
	 */
	@Override
	public List<SiteResDto> searchByQuery(String query, int begin, int size) {
		...
		SearchResponse response = esclient.getData(begin, size, ESConstant.ES_SITE_INFO.INDEX.getInfo(),
				ESConstant.ES_SITE_INFO.TYPE.getInfo(), query);
	...
	public List<SiteResDto> searchRecent() {
		List<SiteResDto> list = Lists.newArrayList();
		// TODO Auto-generated method stub
		SearchResponse response = esclient.getRecentData(ESConstant.ES_SITE_INFO.INDEX.getInfo(),
				ESConstant.ES_SITE_INFO.TYPE.getInfo());
		SearchHits hits = null;
		long time = System.currentTimeMillis();
		if (response != null) {
			hits = response.getHits();
